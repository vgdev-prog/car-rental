...
(function ($, undefined) {
    $(function () {
        ...
        if ($frmCreate.length > 0 || $frmUpdate.length > 0) {
            ...
            $("#updateCar").bind("click", function (e) {
                if ($dialogUpdateCar.length > 0 && dialog) {
                    ...
                    car_id = $("#car_id").val();
                    if (car_id) {
                        $.get("index.php?controller=pjAdminBookings&action=pjActionGetCarMileageMsg", {
                            car_id: car_id,
                            mileage: $('#end').val()
                        }, function (data) {
        ...
        if ($("#grid").length > 0 && datagrid) {
            ...
            var $grid = $("#grid").datagrid({
                ...
                columns: [
                    ...
                    {
                        text: myLabel.insurance_type,
                        type: "select",
                        sortable: false,
                        editable: true,
                        width: 70,
                        options: [
                            {label: myLabel.assurance, value: "assurance"},
                            {label: myLabel.walkin, value: "walkin"}
                        ],
                        applyClass: "pj-insurance_type"
                    },
                    {
                        text: myLabel.bill_for_insurance,
                        type: "select",
                        sortable: true,
                        editable: true,
                        width: 70,
                        options: [
                            {label: myLabel.bill, value: "bill"},
                            {label: myLabel.notbill, value: "notbill"}
                        ],
                        applyClass: "pj-bill_for_insurance"
                    },
                    {
                        text: myLabel.booking_ref, type: "checkbox", sortable: true, editable: false, width: 60
                    },
                    {
                        text: myLabel.booking_insurance_flag,
                        type: "checkbox-flag",
                        sortable: true,
                        editable: false,
                        width: 60
                ...
                dataUrl:
                    isRedirected
                        ?
                        "index.php?controller=pjAdminBookings&action=pjActionGetBookings&insurance_type=assurance" + '&' + gridQuery
                        :
                        "index.php?controller=pjAdminBookings&action=pjActionGetBookings&insurance_type=assurance",
                dataType: "json",
                fields: ['booking_id', 'pick_drop_actual', 'type', 'car_info', 'client', 'total_price', 'status', 'booking_payment_status', 'insurance_type', 'bill_for_insurance', 'ref', 'insurance_flag', 'call', 'p_t', 'cancel'],
                paginator: {
                    gotoPage: true,
                    paginate: true,
                    total: true,
        ...
        $(document).on("click", ".btn-all", function (e) {
            ...
            $grid.datagrid("load", "index.php?controller=pjAdminBookings&action=pjActionGetBookings&insurance_type=assurance" + pjGrid.queryString, "created", "DESC", content.page, content.rowCount);
            return false;
            ...
            $.extend(cache, {
                ...
                status: "",
                p_t: $this.find("input[name='p_t']").val(),
                paid: $this.find("input[name='paid']").val(),
                cancel: $this.find("input[name='cancel']").val(),
                type_id: "",
                booking_id: "",
                pickup_from: "",
                page: content.page,
            ...
            $grid.datagrid("option", "cache", cache);
            $grid.datagrid("load", "index.php?controller=pjAdminBookings&action=pjActionGetBookings&insurance_type=assurance", "created", "DESC", content.page, content.rowCount);
            return false;
        ...
        }).on('change', '.table-checkbox', function (e) {
            let value = this.checked ? 1 : 0;
            let id = $(this).attr("data-id");
            let customer_id = $(this).attr("data-customer-id");
            let column = $(this).attr("data-column");
            $.post("index.php?controller=pjAdminBookings&action=pjActionUpdateBookingValue", {
                id: id,
                [column]: value
            }, function () {
                $grid.datagrid("load", "index.php?controller=pjAdminBookings&action=pjActionGetBookings&insurance_type=assurance" + pjGrid.queryString, "created", "DESC", content.page, content.rowCount);
            });
        }).on('change', '.table-checkbox-flag', function (e) {
            let insurance_flag = this.checked ? 1 : 0;
            let id = $(this).attr("data-id");
            let customer_id = $(this).attr("data-customer-id");
            $.post("index.php?controller=pjAdminBookings&action=pjActionUpdateBookingValue", {
                id: id,
        ...
        }).on('change', '.table-checkbox-call ', function (e) {
            let call = this.checked ? 1 : 0;
            let id = $(this).attr("data-id");
            $.post("index.php?controller=pjAdminBookings&action=pjActionUpdateBookingValue", {
                id: id,
                call: call
