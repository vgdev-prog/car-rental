    ...
    $(function () {
            ...
            $frmCreateReference.validate({
                rules: {
                    "email": {
                        required: true,
                        email: true,
                        remote: "index.php?controller=pjAdminInsuranceCompany&action=pjActionCheckEmail"
                    }
                },
                messages: {
                    "email": {
            ...
            $frmUpdateReference.validate({
                rules: {
                    "email": {
                        required: true,
                        email: true,
                        remote: "index.php?controller=pjAdminInsuranceCompany&action=pjActionCheckEmail&id=" + $frmUpdateReference.find("input[name='id']").val()
                    }
                },
                messages: {
                    "email": {
                ...
                buttons: {
                    "Delete": function () {
                        var $this = $(this),
                            $link = $this.data("link"),
                            $tr = $link.closest("tr");
                        $.post("index.php?controller=pjAdminInsuranceCompany&action=pjActionDelete", {
                            id: $link.data("id"),
                            Qid: $link.data("id"),
                        }).done(function () {
        ...
        if ($("#grid").length > 0 && datagrid) {
            ...
            var $grid = $("#grid").datagrid({
                buttons: [
                    {type: "edit", url: "index.php?controller=pjAdminInsuranceCompany&action=pjActionUpdate&id={:id}"},
                    {type: "delete", url: "index.php?controller=pjAdminInsuranceCompany&action=pjActionDelete&id={:id}"}
                ],
                columns: [
                ...
                dataUrl: "index.php?controller=pjAdminInsuranceCompany&action=pjActionGetInsuranceCompanies",
                dataType: "json",
                fields: ['name','status'],
                paginator: {
                    gotoPage: true,
                ...
                saveUrl: "index.php?controller=pjAdminInsuranceCompany&action=pjActionSave&id={:id}"
            });
        }
